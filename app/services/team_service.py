...
class TeamService:
    ...
    def get_all_teams(self, league_name: str = None, season: str = None):
        ...
        adapter = self.server.data_adapter
        if hasattr(adapter, "get_all_teams"):
            return adapter.get_all_teams(league_name=league_name, season=season)
        ...
        df = adapter.get_filtered_data(filters=filters, columns=[Columns.team_name])
