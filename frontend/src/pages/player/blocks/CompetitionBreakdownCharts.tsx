...
import {
  type BreakdownPieColorMode,
  breakdownChartLabel,
  breakdownSliceColor,
  breakdownSortedByAverage,
  buildBreakdownColorById,
  buildCompetitionBreakdown,
  buildGamesSharePieSlices,
  formatBreakdownTooltipHtml,
...
type Props = {
  ...
  /** Pie palette: ``matchBar`` (default) reuses bar colors per competition; ``sliceOrder`` uses slice index. */
  pieColorMode?: BreakdownPieColorMode;
};

export function CompetitionBreakdownCharts({
  seasons,
  t,
  formatCompetition,
  pieColorMode = "matchBar",
}: Props) {
  ...
  const pieSlices = useMemo(() => buildGamesSharePieSlices(byAverage), [byAverage]);
  const colorById = useMemo(() => buildBreakdownColorById(byAverage), [byAverage]);

  const averageOption = useMemo<EChartsOption | null>(() => {
    if (byAverage.length === 0) return null;

    const names = byAverage.map((e) => breakdownChartLabel(e, formatCompetition));
    ...
    return {
      ...
      tooltip: {
        ...
        formatter: (raw: unknown) => {
          ...
          if (!entry) return "";
          return formatBreakdownTooltipHtml(entry, t, formatCompetition);
        },
      },
      ...
      series: [
        {
          ...
          data: averages.map((value, idx) => ({
            value,
            itemStyle: { color: colorById.get(byAverage[idx]?.id ?? "") },
          })),
          barMaxWidth: 22,
        },
      ],
    };
  }, [byAverage, colorById, compactLayout, formatCompetition, t]);
      ...
      return {
        ...
        chartName:
          slice.id === "__other__"
            ? slice.name
            : source
              ? breakdownChartLabel(source, formatCompetition)
              : slice.name,
      };
    });
    ...
    return {
      ...
      tooltip: {
        ...
        formatter: (raw: unknown) => {
          ...
          if (meta?.entry) {
            return formatBreakdownTooltipHtml(meta.entry, t, formatCompetition);
          }
          const heading = meta?.chartName ?? p.name ?? "";
          return [
      ...
      series: [
        {
          ...
          data: sliceMeta.map((slice, idx) => ({
            ...
            value: slice.games,
            itemStyle: {
              color: breakdownSliceColor(slice.id, idx, colorById, pieColorMode),
            },
          })),
        },
      ],
    };
  }, [breakdown, colorById, compactLayout, formatCompetition, pieColorMode, pieSlices, t]);
